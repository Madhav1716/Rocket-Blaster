...
import { drawMenu, type MenuSection } from "./MenuScreen";
...
export default function RocketBlaster() {
  ...
  // drawLogo moved to MenuScreen.ts
  ...
  function draw(ctx: CanvasRenderingContext2D, width: number, height: number) {
    ...
    if (mode === "menu") {
      ...
      // Backdrop (monochrome) with subtle vignette
      ctx.save();
      ...
      // Panel (soft outline with inner glow)
      ...
      // Delegated menu/welcome rendering
      drawMenu(ctx, width, height, {
        themeName: themeA.name,
        currentSection: menuSectionRef.current,
        aimPosition: aimPositionRef.current,
        hitRects: menuHitRectsRef.current,
        setPointerCursor: (pointer) => {
          if (canvasRef.current) canvasRef.current.style.cursor = pointer ? "pointer" : "default";
        },
      });
